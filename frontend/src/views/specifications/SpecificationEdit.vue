...
import { getSpecificationDetail, updateSpecification } from '@/api/specification'
...
const loadSpecificationDetail = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      form.options = data.options && data.options.length > 0 
        ? [...data.options] 
        : [{ label: '', value: '' }]
      form.minValue = data.minValue || null
      form.maxValue = data.maxValue || null
...
const handleSave = async () => {
  try {
    ...
    const response = await updateSpecification(specificationId.value, submitData)
    if (response.success) {
      ElMessage.success('规格更新成功')
      router.push('/specifications/list')
    } else {
      ElMessage.error(response.message || '规格更新失败')
    }
  } catch (error) {
